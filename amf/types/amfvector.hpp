...
public:
	AmfVector() : values({}), fixed(false) { valueType = static_cast<AmfMarker>(VectorProperties<T>::marker); }
	AmfVector(std::vector<T> vector, bool fixed = false) :
		values(vector), fixed(fixed) { valueType = static_cast<AmfMarker>(VectorProperties<T>::marker); }
...
public:
	AmfVector(std::string type, bool fixed = false) : type(type), fixed(fixed) { valueType = AMF_VECTOR_OBJECT; }
