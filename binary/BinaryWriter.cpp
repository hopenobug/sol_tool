...
void BinaryWriter::Write(const std::vector<char> &data) {
    if (!data.empty()) {
        WriteFromMemory(&data[0], data.size());
    }    
}

void BinaryWriter::Write(const std::vector<unsigned char> &data) {
    if (!data.empty()) {
        WriteFromMemory(&data[0], data.size());
    }    
}

void BinaryWriter::SeekBeg(size_t absoluteOffset) { stream_->seekp(absoluteOffset, std::ifstream::beg); }
